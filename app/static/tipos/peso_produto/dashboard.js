(function () {
  ...
  function corParaIndice(indice, total) {
    ...
    // o produto é texto livre, quantidade variável.
    const faixaUtil = 300;
    ...
    const linhas = registros
      ...
      .map((r) => {
        ...
        return `<tr class="${classeLinha}">
          ...
          <td>${r.horario}</td>
          <td>${r.produto}</td>
          <td>${r.peso_liquido_nominal}</td>
          <td>${r.peso_embalagem}</td>
  ...
  async function carregarDados() {
    ...
    elBotaoExportar.href = "/dashboard/peso_produto/exportar?" + qs;

    const resposta = await fetch("/dashboard/peso_produto/api?" + qs);
