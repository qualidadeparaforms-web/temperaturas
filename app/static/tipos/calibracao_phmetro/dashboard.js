(function () {
    ...
    const linhas = registros
      ...
      .map((r) => {
        ...
        return `<tr class="${classeLinha}">
          <td>${r.data}</td>
          <td>${r.calibracao_realizada_texto}</td>
          <td>${r.responsavel}</td>
          <td>${badge}</td>
        </tr>`;
      })
  ...
  function atualizarGrafico(registros) {
    const ctx = document.getElementById("graficoCalibracaoPhmetro");
    const { min: minEixoX, max: maxEixoX } = calcularLimitesEixoX(registros);

    // Uma unica linha — y = 1 quando a calibracao NAO foi realizada
    // (vermelho), 0 quando foi.
    ...
    grafico = new Chart(ctx, {
      ...
      data: {
        datasets: [
          {
            label: "Calibração Realizada",
            data: pontos,
            borderColor: AZUL,
            backgroundColor: AZUL,
            stepped: true,
            pointRadius: pontos.map((p) => (p.conforme ? 3 : 6)),
            pointBackgroundColor: pontos.map((p) => (p.conforme ? AZUL : VERMELHO)),
            pointBorderColor: pontos.map((p) => (p.conforme ? AZUL : VERMELHO)),
          },
        ],
      },
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              ...
              label: function (item) {
                const p = item.raw;
                return p.conforme ? "Calibração realizada" : "⚠️ Calibração NÃO realizada";
              },
            },
          },
        },
        scales: {
          ...
          y: {
            ...
            ticks: {
              stepSize: 1,
              callback: function (valor) {
                return valor === 1 ? "Não" : "Sim";
              },
            },
            title: { display: true, text: "Calibração realizada?" },
          },
        },
      },
    });
  ...
  async function carregarDados() {
    ...
    elBotaoExportar.href = "/dashboard/calibracao_phmetro/exportar?" + qs;

    const resposta = await fetch("/dashboard/calibracao_phmetro/api?" + qs);
