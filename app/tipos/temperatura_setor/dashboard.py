...
dashboard_bp = Blueprint(
    "temperatura_setor_dashboard", __name__, url_prefix="/dashboard/temperatura_setor"
)
...
def buscar_registros(periodo: str, setor: str, data_inicio: str | None = None, data_fim: str | None = None):
    ...
    query = RegistroTemperaturaSetor.query.filter(
        RegistroTemperaturaSetor.data >= inicio, RegistroTemperaturaSetor.data <= fim
    )
    if setor and setor != "todos":
        query = query.filter(RegistroTemperaturaSetor.setor == setor)

    registros = query.order_by(
        RegistroTemperaturaSetor.data.asc(), RegistroTemperaturaSetor.horario.asc()
    ).all()
...
def index():
    ...
    return render_template(
        "tipos/temperatura_setor/dashboard.html", setores=SETORES, tipos=TIPOS_REGISTRO
    )
...
def adicionar_planilha(wb: Workbook, inicio: date, fim: date, filtros_extra: dict | None = None) -> None:
    ...
    de setor/câmara do período. Sempre cria uma aba nova — usada tanto
    pela exportação exclusiva deste tipo quanto pela combinada."""
    filtros_extra = filtros_extra or {}
    setor = filtros_extra.get("setor", "todos")
    ...
    ws = wb.create_sheet("Temperatura Setor")

    colunas = ["Dia", "Horário", "Setor", "Temperatura", "Responsável", "Conforme"]
    ws.append(colunas)
...
def exportar():
    ...
    wb = Workbook()
    del wb["Sheet"]
    adicionar_planilha(wb, inicio, fim, {"setor": filtros["setor"]})
    ...
    nome_arquivo = (
        f"registros_temperatura_setor_{inicio.strftime('%Y%m%d')}_{fim.strftime('%Y%m%d')}.xlsx"
    )
    return send_file(
        buffer,
        as_attachment=True,
...
def contar(inicio: date, fim: date) -> int:
    return RegistroTemperaturaSetor.query.filter(
        RegistroTemperaturaSetor.data >= inicio, RegistroTemperaturaSetor.data <= fim
    ).count()
...
def linhas_combinadas(inicio: date, fim: date) -> list:
    registros = (
        RegistroTemperaturaSetor.query.filter(
            RegistroTemperaturaSetor.data >= inicio, RegistroTemperaturaSetor.data <= fim
        )
        .order_by(RegistroTemperaturaSetor.data.asc(), RegistroTemperaturaSetor.horario.asc())
        .all()
    )
    return [
        {
            "tipo": "Temperatura de Setor/Câmara",
            "icone": "🧊",
            "data": r.data.strftime("%d/%m/%Y"),
            "horario": r.horario.strftime("%H:%M"),
            "resumo": f"{r.setor}: {r.temperatura:.1f}°C",
            "conforme": r.conforme,
