...
from app.tipos.gramatura.models import RegistroGramatura

dashboard_bp = Blueprint("gramatura_dashboard", __name__, url_prefix="/dashboard/gramatura")
...
def buscar_registros(periodo: str, data_inicio: str | None = None, data_fim: str | None = None):
    ...
    registros = (
        RegistroGramatura.query.filter(
            RegistroGramatura.data >= inicio, RegistroGramatura.data <= fim
        )
        .order_by(RegistroGramatura.data.asc(), RegistroGramatura.horario.asc())
        .all()
    )
...
def index():
    ...
    return render_template("tipos/gramatura/dashboard.html", tipos=TIPOS_REGISTRO)
...
def adicionar_planilha(wb: Workbook, inicio: date, fim: date, filtros_extra: dict | None = None) -> None:
    ...
    ws_resumo = wb.create_sheet("Gramatura")
    colunas_resumo = [
        "Dia",
        "Produto",
        "Gramatura Mínima",
    ...
    ws_detalhe = wb.create_sheet("Gramatura Detalhe")
...
def exportar():
    ...
    nome_arquivo = f"registros_gramatura_{inicio.strftime('%Y%m%d')}_{fim.strftime('%Y%m%d')}.xlsx"
    return send_file(
        buffer,
        as_attachment=True,
...
def contar(inicio: date, fim: date) -> int:
    return RegistroGramatura.query.filter(
        RegistroGramatura.data >= inicio, RegistroGramatura.data <= fim
    ).count()
